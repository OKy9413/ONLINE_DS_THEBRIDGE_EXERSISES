...
def get_features_num_regression(df, target_col, umbral_corr=0.5, pvalue=None):
    """
    Devuelve dos listas con las columnas numéricas del dataframe cuya correlación con la columna designada por "target_col" 
    es menor en valor absoluto al valor dado por "umbral_corr", separándolas en correlaciones positivas y negativas.

    Args:
        df: El DataFrame del que se quiere obtener las características con baja correlación.
        target_col: El nombre de la columna objetivo.
        umbral_corr: cantidad numérica entre 0 a 1.
        pvalue: El valor p para filtrar características significativas basadas en su correlación.
    
    Returns:
        Dos listas: la primera con las columnas de correlación baja positiva, y la segunda con las columnas de correlación baja negativa.
    """
    ...
    # Calculamos las correlaciones y p-values utilizando f_regression
    X = df_num.drop(columns=[target_col])
    y = df_num[target_col]
    _, p_values = f_regression(X, y)

    # Obtenemos las correlaciones excluyendo la variable objetivo
    correlaciones = df_num.corr()[target_col].drop(target_col)  # Excluimos la target de las correlaciones

    # Filtramos las columnas con correlaciones menores en magnitud que el umbral_corr
    columnas_baja_corr = correlaciones[abs(correlaciones) < umbral_corr]

    # Diferenciamos entre correlaciones positivas y negativas
    columnas_baja_corr_pos = columnas_baja_corr[columnas_baja_corr > 0].index.tolist()
    columnas_baja_corr_neg = columnas_baja_corr[columnas_baja_corr < 0].index.tolist()

    if pvalue is not None:
        columnas_significativas = [col for col, p_val in zip(X.columns, p_values) if p_val <= pvalue]
        columnas_baja_corr_pos = [col for col in columnas_baja_corr_pos if col in columnas_significativas]
        columnas_baja_corr_neg = [col for col in columnas_baja_corr_neg if col in columnas_significativas]

    print(f"Columnas con correlación positiva y |valor| < {umbral_corr}: {columnas_baja_corr_pos}")
    print(f"Columnas con correlación negativa y |valor| < {umbral_corr}: {columnas_baja_corr_neg}")
    
    return columnas_baja_corr_pos + columnas_baja_corr_neg
